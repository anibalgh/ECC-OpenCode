...
```bash
mkdir -p ~/.opencode/session-data
```

### Step 3: Write the session file

Create `~/.opencode/session-data/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:
...
- The file is meant to be read by OpenCode at the start of the next session via `/resume-session`
- Use the canonical global session store: `~/.opencode/session-data/`
