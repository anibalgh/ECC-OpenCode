...
1. Default to dry-run. Do not modify `AGENTS.md`, settings files, rules, skills, or install state until the user approves the concrete plan.
2. Preserve existing project guidance. If `AGENTS.md`, `.claude/settings.local.json`, `.cursor/`, `.codex/`, `.gemini/`, `.opencode/`, `.codebuddy/`, `.joycode/`, or `.qwen/` already exists, inspect it and propose a merge/append plan instead of overwriting.
...
1. Identify the target harness. Default to `opencode` unless the user asks for `cursor`, `codex`, `gemini`, `opencode`, `codebuddy`, `joycode`, or `qwen`.
...
If the user wants a `AGENTS.md` starter, generate it separately from the installer plan and keep it minimal:
...
Never replace an existing `AGENTS.md` without showing a diff and receiving approval.
