...
- Any time you have a session file and want OpenCode to fully absorb it before proceeding

## Usage

```
/resume-session                                                      # loads most recent file in ~/.opencode/session-data/
/resume-session 2024-01-15                                           # loads most recent session for that date
/resume-session ~/.opencode/session-data/2024-01-15-abc123de-session.tmp  # loads a current short-id session file
/resume-session ~/.opencode/sessions/2024-01-15-session.tmp               # loads a specific legacy-format file
```
...
4. If the folder does not exist or has no eligible matching files, tell the user:
   ```
   No session files found in ~/.opencode/session-data/
...
- If it looks like a date (`YYYY-MM-DD`), search `~/.opencode/session-data/` first, then the legacy
  `~/.opencode/sessions/`, for files matching `YYYY-MM-DD-session.tmp` (legacy format) or
...
```
SESSION LOADED: /Users/you/.opencode/session-data/2024-01-15-abc123de-session.tmp
════════════════════════════════════════════════
